...
class VectorStore:
    ...
    def __init__(self,
        ...
        # 初始化 BGE-M3 嵌入函数，使用 GPU 设备并启用 FP16
        self.embedding_function = BGEM3EmbeddingFunction(model_name_or_path=conf.M3_MODEL_PATH, use_fp16=True,
                                                         device='cuda:0')
        # 获取稠密向量的维度
    ...
    def _create_or_load_collection(self):
        ...
        if not self.client.has_collection(self.collection_name):
            ...
            # 添加文本字段（子块），VARCHAR 类型，最大长度 65535
    ...
    def add_documents(self, documents):
        ...
        print(embeddings)
        # 转成 csr_matrix，避免 SciPy 1.14+ 已移除 csr_array.getrow 的问题
        sparse_matrix = csr_matrix(embeddings["sparse"])
        # 初始化空列表，存储插入的数据
        data = []
        # 遍历每个文档，带上索引i
    ...
    def hybrid_search_with_rerank(self, query, k=conf.RETRIEVAL_K, source_filter=None):
        ...
        sparse_query_vector = {}
        # 获取查询稀疏向量的第 0 行数据
        row = csr_matrix(query_embeddings["sparse"])[0]
        # 获取稀疏向量的非零值索引
        indices = row.indices
        # 获取稀疏向量的非零值
        values = row.data
...
if __name__ == '__main__':
    ...
    doc = process_documents(data_directory)
    print(len(doc))
