...
class RAGSystem:
    ...
    def _generate_answer_stream(self, prompt):
        response_stream = self.stream_llm(prompt)
    ...
    def stream_answer(self, query, source_filter=None, history=None):
        ...
        if query_category == "通用知识":
            ...
            try:
                yield from self._generate_answer_stream(prompt_input)
            except Exception as e:
...
if __name__ == '__main__':
    ...
    RAGSystem = RAGSystem(vector_store,llm)
    query = 'AI学科学费是多少？'
    print(RAGSystem.generate_answer(query))
